...
@st.cache_data(ttl=60) # Caches the data for 60 seconds so it doesn't over-query Notion
def fetch_notion_data(token, db_id):
    ...
    # Parse Notion JSON into a clean list of dicts
    parsed_data = []
    for r in results:
        ...
        parsed_data.append({
            ...
            "Market Score": get_text("Market Score (1-10)"),
            "Traction Score": get_text("Traction Score (1-10)"),
            "Founder Score": get_text("Founder Score (1-10)"),
            "Position Score": get_text("Position Score (1-10)"),
            "Quona Score": get_text("Quona Score"),
...
with tab1:
    ...
    if notion_df.empty:
        ...
        companies = sorted(notion_df["Company Name"].dropna().unique())
        selected_company = st.selectbox("Select a company to view the breakdown:", companies)
        ...
        with col_metrics:
            ...
            m_score = float(row.get("Market Score", 0) or 0)
            t_score = float(row.get("Traction Score", 0) or 0)
            f_score = float(row.get("Founder Score", 0) or 0)
            p_score = float(row.get("Position Score", 0) or 0)

            st.metric("Total Quona Score", round(float(row.get("Quona Score", 0) or 0), 2))
...
with tab2:
    ...
    - Stage: Seed raised 1 to 3 years ago (NO Series A).
    ...
    if 'llm_results' in st.session_state:
        ...
        if st.button("📤 Push Approved to Notion"):
            ...
            with st.spinner("Syncing..."):
                ...
                # Clear the cache so the new deals appear in the Viewer immediately
